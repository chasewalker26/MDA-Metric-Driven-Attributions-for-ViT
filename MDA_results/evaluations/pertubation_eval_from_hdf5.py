...
from data.expl_hdf5 import ImagenetResults
...
def eval(args):
    ...
    n_correct = "Num correct: " + " " + str(np.mean(num_correct_model)) + ", "  + str(np.std(num_correct_model))
    dissim = "Dissimilarity: " + " " + str(np.mean(dissimilarity_model)) + ", "  + str(np.std(dissimilarity_model))
    pert_steps = "Perturbation steps: ", perturbation_steps
    n_correct_pert = "Num correct perturb: " + " " + str(np.mean(num_correct_pertub, axis=1)) + ", "  + str(np.std(num_correct_pertub, axis=1))
    dissim_perturb = "Dissimilarity perturb: " + " " + str(np.mean(dissimilarity_pertub, axis=1)) + ", "  + str(np.std(dissimilarity_pertub, axis=1))

    auc_score = auc(np.mean(num_correct_pertub, axis=1))
    print(auc_score)

    with open(args.experiment_dir + "/result.csv", 'w') as f:
        write = csv.writer(f)
        write.writerow([n_correct])
        write.writerow([dissim])
        write.writerow([pert_steps])
        write.writerow([n_correct_pert])
        write.writerow([dissim_perturb])
        write.writerow(["auc: ", auc_score])
...
if __name__ == "__main__":
    ...
    if args.vis_class == 'index':
        args.runs_dir = os.path.join(PATH, 'experiments/perturbations/{}/{}/{}_{}'.format(args.model,
                                                                                       exp_name,
                                                                                       args.vis_class,
                                                                                       args.class_id))
    else:
        ablation_fold = 'ablation' if args.is_ablation else 'not_ablation'
        args.runs_dir = os.path.join(PATH, 'experiments/perturbations/{}/{}/{}/{}'.format(args.model,
                                                                                    exp_name,
                                                                                    args.vis_class, 
                                                                                    ablation_fold))
    ...
    if args.vis_class == 'index':
        vis_method_dir = os.path.join(PATH,'visualizations/{}/{}/{}_{}'.format(args.model,
                                                          args.method,
                                                          args.vis_class,
                                                          args.class_id))
    else:
        ablation_fold = 'ablation' if args.is_ablation else 'not_ablation'
        vis_method_dir = os.path.join(PATH,'visualizations/{}/{}/{}/{}'.format(args.model,
                                                        args.method,
                                                        args.vis_class, 
                                                        ablation_fold))
