...
def run_and_save_tests(img_hw, image_count, function, transform, model, explainer, LRP_explainer, model_name, device, imagenet, batch_size):
    ...
    for image in images:    
        ...
        if (function == 'IG'):  
            ...
            measured_time = time.time() - start2
        elif (function == 'GC'):    
            start2 = time.time()
            ...
            # result, features = ViT_CX(model.to("cpu"), img_tensor.to("cpu"), target_layer, gpu_batch=1, device = "cpu")
            ...
            attr = svs.attribute(img_tensor, target=target_class, feature_mask=segments.reshape(1, 1, 224, 224).cuda()).squeeze().permute((1, 2, 0))
            ...
            saliency_map = upsize(downsize(attr.permute((2, 0, 1)))).permute((1, 2, 0))
            measured_time = time.time() - start2
            ...
            saliency_map, _ = MDAFunctions.find_deletion_patches(img_tensor.cpu(), segments, saliency_map_segmented, order_a[0 : end_index + 1], blur, segment_count, model, device, img_hw, max_batch_size = batch_size)
            measured_time = time.time() - start2
        else:
    ...
    folder = "test_results_time/imagenet/"
    ...
    with open(folder + img_label + ".csv", 'w') as f:
        write = csv.writer(f)
        write.writerow(["avg: " + str(avg)])
        write.writerow(["std: " + str(std)])
