...
def run_and_save_tests(img_hw, image_count, function, transform, model, explainer, LRP_explainer, model_name, device, imagenet, batch_size, cutoff = 0.9, kappa = 0.5):
    ...
    fields = ["attr", "SIC", "AIC", "Rise Ins", "Rise Del", "Rise Ins - Del", "MAS Ins", "MAS Del", "MAS Ins - Del"]
    if function == "Calibrate":
        scores = [function, 0, 0, 0, 0, 0, 0, 0, 0]
        scores_smooth = [function + " Smooth", 0, 0, 0, 0, 0, 0, 0, 0]
        scores_b = [function + " gamma 0.5", 0, 0, 0, 0, 0, 0, 0, 0]
        scores_smooth_b = [function + " gamma 0.5 Smooth", 0, 0, 0, 0, 0, 0, 0]
        scores_c = [function + " gamma 1", 0, 0, 0, 0, 0, 0, 0, 0]
        scores_smooth_c = [function + " gamma 1 Smooth", 0, 0, 0, 0, 0, 0, 0, 0]
    else:
        scores = [function, 0, 0, 0, 0, 0, 0, 0, 0]
    ...
    for image in images:    
        ...
        if (function == 'IG' ):    
            ...
            saliency_map, saliency_map_smooth, saliency_map_b, saliency_map_smooth_b, saliency_map_c, saliency_map_smooth_c, _ = MDAFunctions.find_deletion_patches(img_tensor.cpu(), segments, saliency_map_segmented, order_a[0 : end_index + 1], blur, segment_count, model, device, img_hw, max_batch_size = batch_size)
            ...
            downsize = transforms.Resize(int(segment_count ** (1/2)))
            saliency_map_segmented = upsize(downsize(torch.tensor(reference_attr, dtype = torch.float).permute((2, 0, 1)))).permute((1, 2, 0))
            saliency_map, saliency_map_smooth, order_a, MR_ins = MDAFunctions.find_insertion_patches(img_tensor.cpu(), saliency_map_segmented, segments, blur, segment_count, type = 1, model = model, device = device, img_hw = img_hw, max_batch_size = batch_size, cutoff = False)
            ...
            downsize = transforms.Resize(int(segment_count ** (1/2)))
            saliency_map_segmented = upsize(downsize(torch.tensor(reference_attr, dtype = torch.float).permute((2, 0, 1)))).permute((1, 2, 0))
            saliency_map, saliency_map_smooth, saliency_map_b, saliency_map_smooth_b, saliency_map_c, saliency_map_smooth_c, _ = MDAFunctions.find_deletion_patches(img_tensor.cpu(), segments, saliency_map_segmented, torch.tensor([]), blur, segment_count, model, device, img_hw, max_batch_size = batch_size)
        # vary tau
        # we test: 0, 0.1, 0.2, ..., 1.0
        elif (function == "Calibrate_Cutoff"):
            segment_img = np.transpose(trans_img.squeeze().detach().numpy(), (1, 2, 0))
            segment_img = img_as_float(segment_img)
            segments = torch.tensor(slic(segment_img, n_segments=segment_count, compactness=10000, start_label=0), dtype = int)
            attr_attr, _ = explainer.bidirectional(img_tensor, target_class, device = device)
            ...
            downsize = transforms.Resize(int(segment_count ** (1/2)))
            saliency_map_segmented = upsize(downsize(torch.tensor(reference_attr, dtype = torch.float).permute((2, 0, 1)))).permute((1, 2, 0))
            saliency_map, _, order_a, MR_ins = MDAFunctions.find_insertion_patches(img_tensor.cpu(), saliency_map_segmented, segments, blur, segment_count, type = 1, model = model, device = device, img_hw = img_hw, max_batch_size = batch_size, cutoff = cutoff)
            if cutoff != 1:
                end_index = np.where(MR_ins >= cutoff)[0][0]
                saliency_map, _, _, _, _, _, _ = MDAFunctions.find_deletion_patches(img_tensor.cpu(), segments, saliency_map_segmented, order_a[0 : end_index + 1], blur, segment_count, model, device, img_hw, max_batch_size = batch_size)
        # vary kappa
        # we test: 0, 0.5, 1, 2, 4, 8, 16, 24
            ...
            _, _, order_a, MR_ins = MDAFunctions.find_insertion_patches(img_tensor.cpu(), saliency_map_segmented, segments, blur, segment_count, type = 1, model = model, device = device, img_hw = img_hw, max_batch_size = batch_size)
            end_index = np.where(MR_ins >= 0.9)[0][0]
            saliency_map, _, _, _, _, _, _ = MDAFunctions.find_deletion_patches(img_tensor.cpu(), segments, saliency_map_segmented, order_a[0 : end_index + 1], blur, segment_count, model, device, img_hw, max_batch_size = batch_size, kappa = (kappa / 100))
        ...
        scores[3] += MAS.auc(raw_score_i)
        scores[4] += MAS.auc(raw_score_d)
        scores[5] += MAS.auc(raw_score_i) - MAS.auc(raw_score_d)
        scores[6] += MAS.auc(corrected_score_i)
        scores[7] += MAS.auc(corrected_score_d)
        scores[8] += MAS.auc(corrected_score_i) - MAS.auc(corrected_score_d)

        if function == "Calibrate":
            saliency_map_test_smooth = np.abs(np.sum(saliency_map_smooth, axis = 2, keepdims=True))

            ins_del_img = img_tensor.cpu()
            _, corrected_score_i, _, _, raw_score_i = MAS_insertion.single_run(ins_del_img, saliency_map_test_smooth, device, max_batch_size = batch_size)
            _, corrected_score_d, _, _, raw_score_d = MAS_deletion.single_run(ins_del_img, saliency_map_test_smooth, device, max_batch_size = batch_size)

            scores_smooth[3] += MAS.auc(raw_score_i)
            scores_smooth[4] += MAS.auc(raw_score_d)
            scores_smooth[5] += MAS.auc(raw_score_i) - MAS.auc(raw_score_d)
            scores_smooth[6] += MAS.auc(corrected_score_i)
            scores_smooth[7] += MAS.auc(corrected_score_d)
            scores_smooth[8] += MAS.auc(corrected_score_i) - MAS.auc(corrected_score_d)

            saliency_map_test_b = np.abs(np.sum(saliency_map_b, axis = 2, keepdims=True))

            ins_del_img = img_tensor.cpu()
            _, corrected_score_i, _, _, raw_score_i = MAS_insertion.single_run(ins_del_img, saliency_map_test_b, device, max_batch_size = batch_size)
            _, corrected_score_d, _, _, raw_score_d = MAS_deletion.single_run(ins_del_img, saliency_map_test_b, device, max_batch_size = batch_size)

            scores_b[3] += MAS.auc(raw_score_i)
            scores_b[4] += MAS.auc(raw_score_d)
            scores_b[5] += MAS.auc(raw_score_i) - MAS.auc(raw_score_d)
            scores_b[6] += MAS.auc(corrected_score_i)
            scores_b[7] += MAS.auc(corrected_score_d)
            scores_b[8] += MAS.auc(corrected_score_i) - MAS.auc(corrected_score_d)

            saliency_map_test_smooth_b = np.abs(np.sum(saliency_map_smooth_b, axis = 2, keepdims=True))

            ins_del_img = img_tensor.cpu()
            _, corrected_score_i, _, _, raw_score_i = MAS_insertion.single_run(ins_del_img, saliency_map_test_smooth_b, device, max_batch_size = batch_size)
            _, corrected_score_d, _, _, raw_score_d = MAS_deletion.single_run(ins_del_img, saliency_map_test_smooth_b, device, max_batch_size = batch_size)

            scores_smooth_b[3] += MAS.auc(raw_score_i)
            scores_smooth_b[4] += MAS.auc(raw_score_d)
            scores_smooth_b[5] += MAS.auc(raw_score_i) - MAS.auc(raw_score_d)
            scores_smooth_b[6] += MAS.auc(corrected_score_i)
            scores_smooth_b[7] += MAS.auc(corrected_score_d)
            scores_smooth_b[8] += MAS.auc(corrected_score_i) - MAS.auc(corrected_score_d)

            saliency_map_test_c = np.abs(np.sum(saliency_map_c, axis = 2, keepdims=True))

            ins_del_img = img_tensor.cpu()
            _, corrected_score_i, _, _, raw_score_i = MAS_insertion.single_run(ins_del_img, saliency_map_test_c, device, max_batch_size = batch_size)
            _, corrected_score_d, _, _, raw_score_d = MAS_deletion.single_run(ins_del_img, saliency_map_test_c, device, max_batch_size = batch_size)

            scores_c[3] += MAS.auc(raw_score_i)
            scores_c[4] += MAS.auc(raw_score_d)
            scores_c[5] += MAS.auc(raw_score_i) - MAS.auc(raw_score_d)
            scores_c[6] += MAS.auc(corrected_score_i)
            scores_c[7] += MAS.auc(corrected_score_d)
            scores_c[8] += MAS.auc(corrected_score_i) - MAS.auc(corrected_score_d)

            saliency_map_test_smooth_c = np.abs(np.sum(saliency_map_smooth_c, axis = 2, keepdims=True))

            ins_del_img = img_tensor.cpu()
            _, corrected_score_i, _, _, raw_score_i = MAS_insertion.single_run(ins_del_img, saliency_map_test_smooth_c, device, max_batch_size = batch_size)
            _, corrected_score_d, _, _, raw_score_d = MAS_deletion.single_run(ins_del_img, saliency_map_test_smooth_c, device, max_batch_size = batch_size)

            scores_smooth_c[3] += MAS.auc(raw_score_i)
            scores_smooth_c[4] += MAS.auc(raw_score_d)
            scores_smooth_c[5] += MAS.auc(raw_score_i) - MAS.auc(raw_score_d)
            scores_smooth_c[6] += MAS.auc(corrected_score_i)
            scores_smooth_c[7] += MAS.auc(corrected_score_d)
            scores_smooth_c[8] += MAS.auc(corrected_score_i) - MAS.auc(corrected_score_d)
    ...
    if function == "Calibrate":
        for i in range(1, 9):
            scores_smooth[i] /= images_used
            scores_smooth[i] = round(scores_smooth[i], 3)
            scores_b[i] /= images_used
            scores_b[i] = round(scores_b[i], 3)
            scores_smooth_b[i] /= images_used
            scores_smooth_b[i] = round(scores_smooth_b[i], 3)
            scores_c[i] /= images_used
            scores_c[i] = round(scores_c[i], 3)
            scores_smooth_c[i] /= images_used
            scores_smooth_c[i] = round(scores_smooth_c[i], 3)

    # make the test folder if it doesn't exist
    folder = "test_results_ins_del_MAS/imagenet/"
    ...
    with open(folder + img_label + ".csv", 'w') as f:
        ...
        if function == "Calibrate":
            write.writerow(scores_smooth)
            write.writerow(scores_b)
            write.writerow(scores_smooth_b)
            write.writerow(scores_c)
            write.writerow(scores_smooth_c)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model',
                        ...
                        help = 'Name of the model to use: VIT_base_32, VIT_base_16, VIT_tiny_16.')
